...
let out=[],id=1;L.forEach(function(l){patterns[l].forEach(function(p,pi){topics.forEach(function(t,ti){out.push({id:"p"+id++,level:l,theme:t[0],text:p.replace("{x}",t[1+((pi+ti)%16)]),tr:"",topic:t[1+((pi+ti)%16)]})})})});return L.flatMap(function(l){return out.filter(function(x){return x.level===l}).slice(0,200)})})();
